...
static void blink_on()
{
  	// time_list
	init_timer(&led_timer);
	led_timer.function = pil_led;
	led_timer.expires = jiffies + HZ/FREQ;
  	add_timer(&led_timer);
}

static void blink_off()
{
...
static ssize_t led_write(struct file *filp,const char *buf,size_t count,loff_t *f_ops)
{
        ...
        char kbuf;
        int ret;
        ret = copy_from_user(&kbuf, (char *)buf,count);//arg = 3-->num = 3
                if (ret != 0)
                {
                        printk("gpio_read: copy_from_user failed\n");
                        return(-EFAULT);
                }
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("DMATEK Co., Ltd <http://www.ittraining.com.tw>")
